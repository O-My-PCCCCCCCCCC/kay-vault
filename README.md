...
备份文件（`.enc`）本身就是加密的，和 `vault.enc` 同一加密强度。导入时会校验密码：

```
导出备份:
  vault.enc ──复制──→ 备份目录/vault-20260701.enc
                      ↑ 文件已经是 AES 加密的，不需要额外操作

导入备份:
  用户选 .enc 文件 + 输入该文件的主密码
  → 用密码 + 文件中的盐值 派生密钥
  → AES-256-GCM 尝试解密
  → 解密失败 → "密码错误，拒绝导入"
  → 解密成功 → 复制到 ~/.key-vault/vault.enc
```

这样即使备份文件被人捡走，不知道密码也导不进去。
